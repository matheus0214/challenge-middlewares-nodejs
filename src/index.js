...
function checksCreateTodosUserAvailability(request, response, next) {
  ...
  if ((user.todos.length < 10 && user.pro === false) || user.pro) return next();

  return response
    .status(403)
    .json({ error: "You is not available to create a new todo" });
}
...
function checksTodoExists(request, response, next) {
  ...
  if (!validate(id))
    return response.status(400).json({ error: "Id is in wrong format" });
...
function findUserById(request, response, next) {
  ...
  const checkUserExist = users.find((user) => user.id === id);
